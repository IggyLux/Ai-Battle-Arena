...
// Derive multiple independent values from the seed hash
// by scrambling with different large primes at each step
...
export class Unit {
    constructor(name, isPlayer = false) {
        ...
        // Each trait gets its own independently derived value so they don't correlate
    ...
    draw(ctx, time, scale = 0.35, showUI = false) {
        // Draw corpse flash before fully removing
        if (this.hp <= 0) {
            if (this.deathFrames < 60) {
                ...
                ctx.beginPath();
                ctx.arc(0, -20 * scale, 30 * scale, 0, Math.PI * 2);
                ctx.fill();
        ...
        if (this.head === "Hooded") {
            ctx.beginPath(); ctx.arc(0, -30, hr + 4, Math.PI, 0); ctx.lineTo(hr + 8, 0); ctx.lineTo(-hr - 8, 0); ctx.closePath(); ctx.fill();
        } else if (this.head === "Beast") {
            ctx.beginPath(); ctx.arc(0, -30, hr, 0, Math.PI * 2); ctx.ellipse(0, -20, hr + 5, 12, 0, 0, Math.PI * 2); ctx.fill();
        } else {
